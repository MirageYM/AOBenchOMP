...
double drand48( void ){
    unsigned int s;
    rand_s( &s );
    double ret = ( double )s / (double)UINT_MAX;
    return ret;
}
#endif
typedef struct _vec
...
render(unsigned char *img, int w, int h, int nsubsamples)
{
    ...
    for( y = 0; y < h; y++ ) {
        #pragma omp parallel for shared( fimg ) private( x, v, u )
        for( x = 0; x < w; x++ ) {
...
main(int argc, char **argv)
{
    ...
    double msec = delta * 1000.0 / ( double )CLOCKS_PER_SEC;

    printf( "total time:%f sec.\n", msec / 1000.0 );
    printf( "average render time:%f sec.\n", msec / 1000.0 / (double)LOOPMAX );
